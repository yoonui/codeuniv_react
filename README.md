...
# 11일차 : 미디어 쿼리
실제 유튜브 페이지는 페이지 소스에서 영상의 width를 담당하는 컴포넌트가 페이지의 크기에 따라 달라진다. 이렇게 화면의 크기에 따라 스타일이 변경되는 웹 페이지를 '반응형 웹'이라고 한다. 구현하는 방법에는 여러가지가 있지만, 여기서는 '미디어 쿼리'를 사용한다.
- 미디어 쿼리란 구체적인 조건으로 스타일을 적용할 수 있도록 확장된 문법이다. 기존의 클래스나 id, 태그에만 적용하던 스타일 방식에서 벗어나, 조건문과 유사하게 스타일링이 가능하다.



# 12일차 : map
map : 배열 객체에 내장된 함수로서, 배열의 요소를 돌면서 인자로 전달된 함수를 사용해 처리된 새로운 결과를 반환하는 함수로, 배열에 적용할 함수를 인자로 받는다.

배열의 요소들로 생성된 자식 컴포넌트들은 고유한 key가 필요하다. 각각의 컴포넌트를 식별할 수 없으면 리액트에서 어떤 컴포넌트의 데이터가 변경되었는지 알아챌 수 없다. 그 결과, 데이터가 변한 컴포넌트면 리렌더링하면 되는 것을 배열의 모든 컴포넌트를 리렌더링 하게 된다. 이를 방지하기 위헤 Key 속성을 추가해줌으로서 컴포넌트에 유일성을 부여해줘야 한다. key 속성은 리턴되는 JSX의 최상단 태그에 부여하면 된다.



# 13일차 : Home과 Filter
filter : map 함수와 유사하게, 배열에 적용해 조건에 맞는 원소들을 모아 새로운 배열을 생성한다. 또한, 조건에 맞으면 True, 아니면 False를 리턴하는 함수를 파라미터를 받는다.



# 14일차 : Moment JS
자바스크립트에서는 기본적인 시간을 계산하는 Date 객체를 제공하고 있다. 하지만 외부 라이브러리 중 하나인 Moment JS의 사용 편의성은 따라갈 수 없다. Moment JS를 설치하는 명령어는 아래와 같다.

$ npm install moment --save

- import 'moment/locale/ko';
- import moment from 'moment';
라이브러리를 사용하기 위해 import를 사용한다. 첫째줄의 moment/locale/ko는 잠시 뒤에 계산할 시간의 차이를 한글로 출력해주며, 두번째 줄은 moment 객체를 불러오는 코드다.

moment 객체에 아무런 인자없이 호출하는 경우, 현재 시간을 리턴해준다. 리턴받은 객체에 year, month, date 등의 메소드를 호출해 각각의 값을 추출해낼 수 있다.

현재 시간을 구할 때 moment 객체에 아무런 인자도 넣지 않았다면, 목표 시간 객체를 얻어오기 위해 인자로 목표 시간 객체를 얻어오기 위해 인자로 시간을 넣어준다. 목표 시간으로부터 현재 시간과의 차이는 moment 라이브러리에서 제공하는 fromNow 함수를 사용해 구할 수 있다.

- toFixed : 인자로 받은 정수 값 아래에서 반올림하고, 해당 결과를 리턴해주는 함수



# 15일차 : 호스팅
만든 클론 코딩 페이지를 호스팅해본다.
- Github 호스팅 : Github는 Git이라고 불리는 분산 버전 관리 시스템을 손쉽게 사용할 수 있게 해주는 서비스이다. 여러 오픈소스 소프트웨어의 허브 역할을 하면서, 여러 프로젝트들이 널리 퍼질 수 있도록 기여하고 있는 서비스로 호스팅을 위한 서비스는 아니다. 다만, Github Pages라는 호스팅 서비스를 제공하고 있다.

- 사전준비
- 첫번째로, HashRouter를 사용해야 한다. 9일차에서 페이지 이동을 위해 index.js 파일에서 BrowserRouter를 사용했다. 그러나 Github Pages는 리액트와 같은 SPA를 제공하지 않는다. 따라서 BrowserRouter를 HashRouter로 변경해준다. HashRouter는 주소값의 Hash(#)를 이용한 라우터이다. 기존의 BrowserRouter가 페이지를 이동시켰다면, HashRouter는 페이지 이동을 요청하는 것이 아닌 현재 페이지에서 # 이후의 요소를 찾아 보여주는 것이다.
- 다음은 public 폴더에 있는 index.html 파일을 변경해준다. 해당 파일이 웹 페이지를 구성했을 때 보여질 토대이다. id가 root인 div 태그 아래에 지금까지 작성했던 모든 코드들이 들어가는 셈이다. 또한, index.html 이외의 파일은 모두 제거해준다.
- 마지막으로 package.json 파일을 수정해준다. json 파일 마지막에 homepage : "." 요소만 추가해준다.

사전 준비를 마쳤다면, 작성했던 코드들을 Build하는 과정을 거쳐야 한다. Build란, 작성한 소스 코드 파일이 컴퓨터나 휴대폰에 실행할 수 있는 파일로 구성하는 과정을 의미한다. 즉, 지금까지 npm start를 사용해야만 볼 수 있었던 웹 페이지를 그냥 열어볼 수 있도록 만드는 과정이다. 아래 명령어를 입력한다.
